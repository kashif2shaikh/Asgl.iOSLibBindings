...
if [ ! -f "Asgl.iOSLibBindings.csproj" ];then
	echo "You must run this script from iOS Lib Binding Project directory";
	exit 1
fi

XCODEPROJ_DIR=${PWD}/../Xcode
POD_DIR=${XCODEPROJ_DIR}/Pods

if [ ! -d $POD_DIR ]; then
	echo "No Pods directory under ${XCODEPROJ_DIR}: You must run pod update"
	exit 1
fi

BUILD_DIR=${XCODEPROJ_DIR}/build
